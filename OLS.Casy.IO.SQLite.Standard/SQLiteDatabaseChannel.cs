...
namespace OLS.Casy.IO.SQLite.Standard
{
    ...
    public class SQLiteDatabaseChannel : IDatabaseStorageProvider, IPartImportsSatisfiedNotification, IDisposable
    {
        ...
        private User GetUser(CasyContext casyContext, int userId)
        {
            ...
            user = new User(userEntity.Username, userRole)
            {
                ...
                RecentTemplateIds = string.IsNullOrEmpty(userEntity.RecentTemplateIds)
                    ? new List<int>()
                    : userEntity.RecentTemplateIds.Split(';')
                        .Select(item => Convert.ToInt32(item, CultureInfo.InvariantCulture)).ToList(),
                FavoriteTemplateIds = string.IsNullOrEmpty(userEntity.FavoriteTemplateIds)
                    ? new List<int>()
                    : userEntity.FavoriteTemplateIds.Split(';')
                        .Select(item => Convert.ToInt32(item, CultureInfo.InvariantCulture)).ToList(),
                SelectedTableColumns = string.IsNullOrEmpty(userEntity.RecentMeasureResultIds)
                    ? new List<string>()
                    : userEntity.RecentMeasureResultIds.Split(';').ToList()
            };
        ...
        public User GetUserByName(string name)
        {
            using (var casyContext = new CasyContext(_environmentService, _activeAuditTrailDecorator, null))
            {
                ...
                var userRole = GetUserRole(userEntity.UserRoleEntityId);
                var userUserGroupMappings = casyContext.UserUserGroupMappings
                    .Where(m => m.UserEntityId == userEntity.UserEntityId).ToList();
        ...
        public void DeleteUser(User user)
        {
            using (var casyContext = new CasyContext(_environmentService, _activeAuditTrailDecorator, null))
            {
                ...
                var userUserGroupMappings =
                    casyContext.UserUserGroupMappings.Where(m => m.UserEntityId == user.Id).ToList();
                foreach (var mapping in userUserGroupMappings)
                {
        ...
        public int SaveUser(User user)
        {
            using (var casyContext = new CasyContext(_environmentService, _activeAuditTrailDecorator, null))
            {
                var userEntity = casyContext.Users.Include("UserRoleEntity")
                    .FirstOrDefault(u => u.UserEntityId == user.Id);

                var userRoleId = user.UserRole == null || user.UserRole == UserRole.None
                    ? 1
                    : casyContext.UserRoles.FirstOrDefault(i => i.Name == user.UserRole.Name).UserRoleEntityId;
        ...
        private void SaveUserUserGroupMapping(CasyContext casyContext, int userId, int userGroupId)
        {
            var userUserGroupMapping =
                casyContext.UserUserGroupMappings.FirstOrDefault(u =>
                    u.UserEntityId == userId && u.UserGroupEntityId == userGroupId);
        ...
        private UserRole GetUserRole(CasyContext casyContext, int userRoleId)
        {
            ...
            userRole = new UserRole
                {Id = userRoleEntity.UserRoleEntityId, Name = userRoleEntity.Name, Priority = userRoleEntity.Priority};
        ...
        private UserGroup GetUserGroup(CasyContext casyContext, int userGroupId, bool fillUser)
        {
            ...
            {
                var userUserGroupMappings = casyContext.UserUserGroupMappings
                    .Where(m => m.UserGroupEntityId == userGroup.Id).ToList();
        ...
        public MeasureSetup GetPredefinedTemplate(string name, int capillarySize)
        {
            using (var casyContext = new CasyContext(_environmentService, _activeAuditTrailDecorator, null))
            {
                var measureSetupIds = casyContext.MeasureSetups
                    .Where(ms => ms.Name == name && ms.IsTemplate && ms.IsReadOnly && ms.CapillarySize == capillarySize)
                    .Select(ms => ms.MeasureSetupEntityId).ToList();

                return measureSetupIds.Count > 0
                    ? GetMeasureSetup(casyContext, measureSetupIds.FirstOrDefault(), true)
                    : null;
            }
        }
        ...
        public bool MeasureResultExists(string name, string experiment, string group)
        {
            using (var casyContext = new CasyContext(_environmentService, _activeAuditTrailDecorator, null))
            {
                return casyContext.MeasureResults.Count(mr =>
                           mr.Name == name && mr.Group == group && mr.Experiment == experiment) > 0;
            }
        }
        ...
        private MeasureSetup GetMeasureSetup(CasyContext casyContext, int measureSetupId, bool ignoreCache = false,
            bool isDeletedResult = false)
        {
            //TODO: Um gelöschte kümmern

            var measureSetup = _measureSetupCache.FirstOrDefault(ms => ms != null && ms.MeasureSetupId == measureSetupId);
        ...
        private MeasureSetupEntity_Deleted DeleteMeasureSetup(CasyContext casyContext, MeasureSetup measureSetup, bool saveContext = true)
        {
            ...
            var measureSetupEntity =
                casyContext.MeasureSetups.FirstOrDefault(ms => ms.MeasureSetupEntityId == measureSetup.MeasureSetupId);
            if (measureSetupEntity == null) return null;
            ...
            {
                var cursorEntity = casyContext.Cursors.Include("MeasureSetupEntity")
                    .FirstOrDefault(c => c.CursorEntityId == cursor.CursorId);
            ...
            {
                var deviationControlItemEntity = casyContext.DeviationControlItems.Include("MeasureSetupEntity")
                    .FirstOrDefault(d => d.DeviationControlItemEntityId == deviationControlItem.DeviationControlItemId);
            ...
            {
                var entity =
                    casyContext.AuditTrailEntries.FirstOrDefault(d =>
                        d.AuditTrailEntryEntityId == auditTrail.AuditTrailEntryId);
        ...
        private void RemoveDeletedMeasureSetup(CasyContext casyContext, int deletedmeasureSetupId)
        {
            //if (measureSetup == null) return null;
        ...
        private MeasureSetupEntity SaveMeasureSetup(CasyContext casyContext, MeasureSetup measureSetup,
            bool ignoreAuditTrail)
        {
            var query = casyContext.MeasureSetups.Include("CursorEntities").Include("DeviationControlItemEntities")
                .Where(ms => ms.MeasureSetupEntityId == measureSetup.MeasureSetupId);
            ...
            var entitiesToDelete = measureSetupEntity.CursorEntities.Where(mse => /*!mse.IsDelete &&*/
                measureSetup.Cursors.All(c => c.CursorId != mse.CursorEntityId)).ToList();
            ...
            var deviationEntitiesToDelete = measureSetupEntity.DeviationControlItemEntities
                .Where(item => /*!item.IsDelete &&*/ measureSetup.DeviationControlItems.All(dci =>
                    dci.DeviationControlItemId != item.DeviationControlItemEntityId)).ToList();
        ...
        private static void SaveCursor(CasyContext casyContext, Cursor cursor, MeasureSetupEntity measureSetupEntity,
        ...
        private static void SaveDeviationControlItem(CasyContext casyContext,
            DeviationControlItem deviationControlItem, MeasureSetupEntity measureSetupEntity,
            bool ignoreAuditTrail = false)
        {
            var deviationControlItemEntity = casyContext.DeviationControlItems.FirstOrDefault(d =>
                d.DeviationControlItemEntityId == deviationControlItem.DeviationControlItemId);
            ...
            if (deviationControlItemEntity == null)
            {
                ...
                if (deviationControlItem.MeasureSetup.IsTemplate)
                {
                    deviationControlItemEntity.AssociatedMeasureResultEntityId = -1;
                }
                else if (deviationControlItem.MeasureSetup.MeasureResult != null)
                {
                    deviationControlItemEntity.AssociatedMeasureResultEntityId = deviationControlItem.MeasureSetup.MeasureResult.MeasureResultId;
                }
                else
                {
                    deviationControlItemEntity.AssociatedMeasureResultEntityId = -1;
        ...
        public IEnumerable<Tuple<string, int, int>> GetExperiments(bool includeDeleted = false)
        {
            using (var casyContext = new CasyContext(_environmentService, _activeAuditTrailDecorator, null))
            {
                ...
                var experiments = experimentQuery.Select(x => new {x.Experiment, x.Group, x.MeasureResultEntityId}).ToList()
                    .GroupBy(x => x.Experiment).ToList();

                if (includeDeleted)
                {
                    var deletedExperimentQuery =
                        casyContext.MeasureResultsDeleted.Where(x => !x.IsTemporary).AsQueryable();

                    experiments = experiments.Union(deletedExperimentQuery
                        .Select(x => new {x.Experiment, x.Group, x.MeasureResultEntityId}).ToList().GroupBy(x => x.Experiment)
                        .ToList()).ToList();
                }
                ...
                foreach (var expGroup in experiments)
                {
                    ...
                    {
                        result[key] = new Tuple<string, int, int>(expGroup.Key, existing.Item2 + expGroup.Count(),
                            existing.Item3 + grpGroup.Count());
                    }
                    else
                    {
        ...
        public IEnumerable<Tuple<string, int>> GetGroups(string experiment, bool includeDeleted = false)
        {
            using (var casyContext = new CasyContext(_environmentService, _activeAuditTrailDecorator, null))
            {
                ...
                var groups = groupQuery.Select(x => new {x.Group, x.MeasureResultEntityId}).ToList().GroupBy(x => x.Group)
                    .ToList();
                ...
                if (includeDeleted)
                {
                    ...
                    groups = groups.Union(groupQueryDeleted.Select(x => new {x.Group, x.MeasureResultEntityId}).ToList()
                        .GroupBy(x => x.Group).ToList()).ToList();
                }
        ...
        public IEnumerable<MeasureResult> GetMeasureResults(string experiment, string group,
            bool includeDeleted = false, bool nullAsNoValue = false)
        {
            using (var casyContext = new CasyContext(_environmentService, _activeAuditTrailDecorator, null))
            {
                ...
                if (includeDeleted)
                {
                    ...
                    if (!string.IsNullOrEmpty(experiment))
                    {
                        measureResultEntitiesDeleted =
                            measureResultEntitiesDeleted.Where(x => x.Experiment == experiment);
                    }
                    else
                    {
                        measureResultEntitiesDeleted =
                            measureResultEntitiesDeleted.Where(x => string.IsNullOrEmpty(x.Experiment));
                    }
        ...
        private static MeasureResult EntityToModel(MeasureResultEntity measureResultEntity)
        {
            ...
            var measureResult = new MeasureResult
            {
                ...
                LastWeeklyClean = string.IsNullOrEmpty(measureResultEntity.LastWeeklyClean)
                    ? null
                    : new DateTimeOffset?(DateTimeOffsetExtensions.ParseAny(measureResultEntity.LastWeeklyClean))
            };
        ...
        private static MeasureResultAnnotation EntityToModel(
            MeasureResultAnnotationEntity measureResultAnnotationEntity)
        {
            ...
            using (var casyContext = new CasyContext(_environmentService, _activeAuditTrailDecorator, null))
            {
                ...
                if (!measureResult.IsDeletedResult)
                {
                    ...
                    lock (((ICollection) measureResult.AuditTrailEntries).SyncRoot)
                    {
                        ...
                        {
                            var auditTrailEntry = GetAuditTrailEntry(casyContext,
                                auditTrailEntryEntity.AuditTrailEntryEntityId);
                ...
                else
                {
                    var measureResultEntity = casyContext.MeasureResultsDeleted
                        .Where(mr => mr.MeasureResultEntityId == id)
                        .Include("MeasureResultDataEntities").Include("MeasureSetupEntity")
                        .Include("OriginalMeasureSetupEntity").Include("AuditTrailEntrieEntities").FirstOrDefault();
                ...
                measureResult.MeasureSetup = GetMeasureSetup(casyContext, measureSetupId,
                    isDeletedResult: measureResult.IsDeletedResult);
                measureResult.MeasureSetup.MeasureResult = measureResult;

                measureResult.OriginalMeasureSetup = GetMeasureSetup(casyContext, origMeasureSetupId,
                    isDeletedResult: measureResult.IsDeletedResult);
        ...
        public void DeleteMeasureResults(IList<MeasureResult> measureResults)
        {
            using (var casyContext = new CasyContext(_environmentService, _activeAuditTrailDecorator, null))
            {
                ...
                foreach (var measureResult in measureResults)
                {
                    ...
                    //var measureResultEntity = casyContext.MeasureResults.FirstOrDefault(mr =>
                        //mr.MeasureResultEntityId == measureResult.MeasureResultId);

                    //casyContext.MeasureResults.Remove(measureResultEntity);

                    var deletedSetup = DeleteMeasureSetup(casyContext, measureResult.MeasureSetup, false);
                    if (deletedSetup != null)
                    {
                        measureResultEntityDeleted.MeasureSetupEntity = deletedSetup;
                    }

                    var deletedOrigSetup = DeleteMeasureSetup(casyContext, measureResult.OriginalMeasureSetup, false);
                    ...
                    //if (measureResultEntity == null) continue;
                    measureResultEntityDeleted.Color = measureResult.Color;
                    measureResultEntityDeleted.Comment = measureResult.Comment;
                    measureResultEntityDeleted.CreatedAt = measureResult.CreatedAt.ToString();
                    measureResultEntityDeleted.CreatedBy = measureResult.CreatedBy;
                    measureResultEntityDeleted.Experiment = measureResult.Experiment;
                    measureResultEntityDeleted.Group = measureResult.Group;
                    measureResultEntityDeleted.IsCfr = measureResult.IsCfr;
                    measureResultEntityDeleted.IsTemporary = measureResult.IsTemporary;
                    measureResultEntityDeleted.Version = measureResult.Version;
                    measureResultEntityDeleted.LastModifiedAt = measureResult.LastModifiedAt.ToString();
                    measureResultEntityDeleted.LastModifiedBy = measureResult.LastModifiedBy;
                    measureResultEntityDeleted.MeasureResultEntityGuid = measureResult.MeasureResultGuid;
                    measureResultEntityDeleted.MeasuredAt = measureResult.MeasuredAt;
                    measureResultEntityDeleted.MeasuredAtTimeZone = measureResult.MeasuredAtTimeZone == null
                        ? TimeZoneInfo.Local.Id
                        : measureResult.MeasuredAtTimeZone.Id;
                    measureResultEntityDeleted.Name = measureResult.Name;
                    measureResultEntityDeleted.Origin = measureResult.Origin;
                    measureResultEntityDeleted.SerialNumber = measureResult.SerialNumber;
                    ...
                    {
                        //var measureResultDataEntity = casyContext.MeasureResultData.FirstOrDefault(mrd =>
                            //mrd.MeasureResultDataEntityId == measureResultData.MeasureResultDataId);
                        ...
                        var measureResultDataDeleted = new MeasureResultDataEntity_Deleted()
                        {
                            DeletedAt = DateTimeOffset.UtcNow.ToString(),
                            DeletedBy = loggedInUser,
                            LastModifiedAt = measureResultData.LastModifiedAt.ToString(),
                            LastModifiedBy = measureResultData.LastModifiedBy,
                            CreatedBy = measureResultData.CreatedBy,
                            CreatedAt = measureResultData.CreatedAt.ToString(),
                            Color = measureResultData.Color,
                    ...
                    {
                        //var entity = casyContext.AuditTrailEntries.FirstOrDefault(d =>
                            //d.AuditTrailEntryEntityId == auditTrail.AuditTrailEntryId);
                        ...
                        var auditTrailDeleted = new AuditTrailEntryEntity_Deleted()
                        {
                            ...
                            ComputerName = auditTrail.ComputerName,
                            DateChanged = auditTrail.DateChanged.ToString(),
                            EntityName = auditTrail.EntityName,
                            NewValue = auditTrail.NewValue,
                            OldValue = auditTrail.OldValue,
                            PrimaryKeyValue = auditTrail.PrimaryKeyValue,
                            PropertyName = auditTrail.PropertyName,
                            SoftwareVersion = auditTrail.SoftwareVersion,
                            UserChanged = auditTrail.UserChanged
                        };
        ...
        public void RemoveDeletedMeasureResult(int deletedMeasureResultId)
        {
            using (var casyContext = new CasyContext(_environmentService, _activeAuditTrailDecorator, null))
            {
                ...
                var measureResultEntityDeleted = casyContext.MeasureResultsDeleted
                    .Include(x => x.MeasureResultDataEntities).Include(x => x.AuditTrailEntrieEntities)
                    .FirstOrDefault(mr => mr.MeasureResultEntityId == deletedMeasureResultId);
                ...
                //RemoveDeletedMeasureSetup(casyContext, measureResultEntityDeleted.MeasureSetupEntity.MeasureSetupEntityId);
                //RemoveDeletedMeasureSetup(casyContext, measureResultEntityDeleted.OriginalMeasureSetupEntity.MeasureSetupEntityId);
        ...
        public int SaveMeasureResult(MeasureResult measureResult, bool ignoreAuditTrail = false,
            bool storeExistingAuditTrail = false)
        {
            using (var casyContext = new CasyContext(_environmentService, _activeAuditTrailDecorator, null))
            {
                var measureResultEntity = casyContext.MeasureResults.Include("MeasureResultDataEntities")
                    .Include("MeasureSetupEntity").Include("OriginalMeasureSetupEntity")
                    .FirstOrDefault(mr => mr.MeasureResultEntityId == measureResult.MeasureResultId);
                ...
                if (measureResultEntity == null)
                {
                    ...
                    var measureSetupEntity =
                        SaveMeasureSetup(casyContext, measureResult.MeasureSetup, ignoreAuditTrail);
                    var origMeasureSetupEntity =
                    ...
                    measureResultEntity = new MeasureResultEntity
                    {
                        ...
                        MeasuredAtTimeZone = measureResult.MeasuredAtTimeZone == null
                            ? TimeZoneInfo.Local.Id
                            : measureResult.MeasuredAtTimeZone.Id,
                    ...
                    if (!ignoreAuditTrail)
                    {
                        ...
                        measureResult.LastModifiedAt =
                            DateTimeOffsetExtensions.ParseAny(measureResultEntity.LastModifiedAt);
                ...
                else
                {
                    ...
                    measureResultEntity.MeasuredAtTimeZone = measureResult.MeasuredAtTimeZone == null
                        ? TimeZoneInfo.Local.Id
                        : measureResult.MeasuredAtTimeZone.Id;
                ...
                var annotationEntitiesToDelete = measureResultEntity.MeasureResultAnnotationEntities.Where(entity =>
                    measureResult.MeasureResultAnnotations.All(mra =>
                        mra.MeasureResultAnnotationId != entity.MeasureResultAnnotationEntityId)).ToList();
        ...
        private static void SaveMeasureResultAccessMapping(CasyContext casyContext,
            MeasureResultAccessMapping measureResultAccessMapping)
        {
            var accessMappingEntity = casyContext.MeasureResultAccessMappings.FirstOrDefault(mr =>
                mr.MeasureResultAccessMappingId == measureResultAccessMapping.MeasureResultAccessMappingId);
            ...
            if (accessMappingEntity == null)
            {
                ...
                casyContext.SaveChanges();
                measureResultAccessMapping.MeasureResultAccessMappingId =
                    accessMappingEntity.MeasureResultAccessMappingId;
            }
            else
            {
        ...
        private static void SaveAuditTrailEntry(CasyContext casyContext, AuditTrailEntry auditTrailEntry)
        {
        ...
        private AnnotationType GetAnnotationType(CasyContext casyContext, int annotationTypeId)
        {
            var annotationTypeEntity =
                casyContext.AnnotationTypes.FirstOrDefault(at => at.AnnotationTypeEntityId == annotationTypeId);
            if (annotationTypeEntity != null)
            {
                return new AnnotationType
                {
        ...
        private static MeasureResultData GetMeasureResultData(CasyContext casyContext, int measureResultDataId,
        ...
        private static void SaveMeasureResultData(CasyContext casyContext, MeasureResultData measureResultData)
        {
            var measureResultDataEntity = casyContext.MeasureResultData.FirstOrDefault(mr =>
                mr.MeasureResultDataEntityId == measureResultData.MeasureResultDataId);
            ...
            if (measureResultDataEntity == null)
            {
                ...
                measureResultData.LastModifiedAt =
                    DateTimeOffsetExtensions.ParseAny(measureResultDataEntity.LastModifiedAt);
            }
            else
            {
        ...
        private static AuditTrailEntry GetAuditTrailEntry(CasyContext casyContext, int auditTrailEntryId)
        {
            var auditTrailEntryEntity =
                casyContext.AuditTrailEntries.FirstOrDefault(ate => ate.AuditTrailEntryEntityId == auditTrailEntryId);
            ...
            {
                var auditTrailEntryEntityDeleted =
                    casyContext.AuditTrailEntriesDeleted.FirstOrDefault(ate =>
                        ate.AuditTrailEntryEntityId == auditTrailEntryId);
        ...
        private static Cursor GetCursor(CasyContext casyContext, int cursorId)
        {
        ...
        public void SaveErrorDetails(ErrorDetails errorDetails)
        {
            using (var casyContext = new CasyContext(_environmentService, _activeAuditTrailDecorator, null))
            {
                var errorDetailsEntity =
                    casyContext.ErrorDetails.Local.FirstOrDefault(ed =>
                        ed.ErrorDetailsEntityId == errorDetails.ErrorDetailsId);
        ...
        public void SaveAnnotationType(AnnotationType annotationType)
        {
            using (var casyContext = new CasyContext(_environmentService, _activeAuditTrailDecorator, null))
            {
                var annotationTypeEntity = casyContext.AnnotationTypes.FirstOrDefault(at =>
                    at.AnnotationTypeEntityId == annotationType.AnnotationTypeId);
        ...
        public void DeleteAnnotationType(AnnotationType annotationType)
        {
            using (var casyContext = new CasyContext(_environmentService, _activeAuditTrailDecorator, null))
            {
                ...
                if (annotationTypeEntity != null)
                {
                    var measureResultAnnotations = casyContext.MeasureResultAnnotations.Where(mra =>
                        mra.AnnotationTypeEntityId == annotationTypeEntity.AnnotationTypeEntityId).ToList();
        ...
        private static void SaveMeasureResultAnnotation(CasyContext casyContext,
            MeasureResultAnnotation measureResultAnnotation, bool ignoreAuditTrail)
        {
            var measureResultAnnotationEntity = casyContext.MeasureResultAnnotations.FirstOrDefault(mra =>
                mra.MeasureResultAnnotationEntityId == measureResultAnnotation.MeasureResultAnnotationId);
            ...
            if (measureResultAnnotationEntity == null)
            {
                ...
                var annotationTypeEntity = casyContext.AnnotationTypes.FirstOrDefault(x =>
                    x.AnnottationTypeName == measureResultAnnotation.AnnotationType.AnnottationTypeName);

                measureResultAnnotationEntity = new MeasureResultAnnotationEntity
                {
                    AnnotationTypeEntity = annotationTypeEntity,
                    MeasureResultEntityId = measureResultEntity.MeasureResultEntityId,
                ...
                measureResultAnnotation.MeasureResultAnnotationId =
                    measureResultAnnotationEntity.MeasureResultAnnotationEntityId;
            }
            else
            {
                measureResultAnnotationEntity.AnnotationTypeEntityId =
                    measureResultAnnotation.AnnotationType.AnnotationTypeId;
        ...
        public Dictionary<string, Setting> GetSettings()
        {
            using (var casyContext = new CasyContext(_environmentService, _activeAuditTrailDecorator, null))
            {
                ...
                {
                    result.Add(settingsEntity.Key,
                        new Setting
                        {
                            Id = settingsEntity.Id, Value = settingsEntity.Value, Key = settingsEntity.Key,
                            BlobValue = settingsEntity.BlobValue
                        });
                }
        ...
        public void CreateBackupFile(string fileName)
        {
            using (var casyContext = new CasyContext(_environmentService, _activeAuditTrailDecorator, null))
            {
                var destination = new SqliteConnection("Data Source=" + fileName + ";");
                destination.Open();
        ...
        public bool RestoreBackupFile(string fileName)
        {
            ...
            {
                var testConnection =
                    new SqliteConnection("Data Source=" + fileName + ";Version=3;");
                testConnection.Open();
                var password = "th1s1sc4sy";
                var command = testConnection.CreateCommand();
                command.CommandText = "SELECT quote($password);";
                command.Parameters.AddWithValue("$password", password);
                var quotedPassword = (string)command.ExecuteScalar();
                ...
                command = new SqliteCommand(@"SELECT * FROM SettingsEntity", testConnection);
                command.ExecuteNonQuery();
        ...
        public void Cleanup(int olderThanDays)
        {
            using (var casyContext = new CasyContext(_environmentService, _activeAuditTrailDecorator, null))
            {
                ...
                foreach (var tuple in auditTrailsToDelete)
                {
                    ...
                    else if (tuple.MeasureSetupEntityId.HasValue && casyContext.MeasureSetupsDeleted.FirstOrDefault(
                                 mr =>
