...
{
    public class SingleMeasureResultDocument : DocumentBase
    {
        ...
        private Paragraph _origOrModSection;
        private Paragraph _lastWeeklyCleanSection;

        public SingleMeasureResultDocument(ILocalizationService localizationService, IAuthenticationService authenticationService,
        ...
        public Document CreateDocument(MeasureResult measureResult, byte[] graphImage, bool showOriginal, bool isCfr)
        {
        ...
        protected void CreatePage(Orientation orientation)
        {
            ...
            if (_isCfr)
            {
                if(ShowLastWeeklyClean)
                {
                    _lastWeeklyCleanSection = Section.AddParagraph();
                    _lastWeeklyCleanSection.Format.Font.Color = Colors.Black;
                    _lastWeeklyCleanSection.Format.Font.Name = "Dosis-Light";
                    _lastWeeklyCleanSection.Format.Font.Size = 9;
                    _lastWeeklyCleanSection.Format.LineSpacing = 10.8;
                    _lastWeeklyCleanSection.Format.LineSpacingRule = LineSpacingRule.AtLeast;
                }

                _origOrModSection = Section.AddParagraph();
                _origOrModSection.Format.Font.Color = Colors.Black;
                _origOrModSection.Format.Font.Name = "Dosis-Light";
                _origOrModSection.Format.Font.Size = 12;
                _origOrModSection.Format.LineSpacing = 14.4;
                _origOrModSection.Format.LineSpacingRule = LineSpacingRule.AtLeast;
            }
        ...
        {
            var measureSetup = _showOriginal ? _measureResult?.OriginalMeasureSetup : _measureResult?.MeasureSetup;

            if (measureSetup?.Cursors == null) return;
            ...
            paragraph.AddText("SINGLE MEASURE");
            ...
            if (_isCfr)
            {
                if(ShowLastWeeklyClean)
                {
                    var lastWeeklyCleanText = _measureResult.LastWeeklyClean == null ? "Unknown" : EnvironmentService.GetDateTimeString(_measureResult.LastWeeklyClean.Value.DateTime);
                    _lastWeeklyCleanSection.AddText($"Last weekly clean before measurement: {lastWeeklyCleanText}");
                }

                if (_showOriginal || !_measureResult.AuditTrailEntries.Any())
                {
                    _origOrModSection.AddText("Original Settings");
                }
                else
                {
                    _origOrModSection.AddText("Modified Settings");
                }
            }
